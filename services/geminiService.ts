...
export const initializeGemini = () => {
  ...
  // Prioritize the key saved in settings, fallback to env variable if available
  const apiKey = keys.geminiApiKey || process.env.API_KEY;
  
  if (!apiKey) {
    throw new Error("Gemini API Key가 없습니다. 우측 상단 [설정] 메뉴에서 API 키를 등록해주세요.");
  }
...
export const generateSuggestions = async (topic: string): Promise<{ target: string; goal: string }> => {
  // Always try to initialize to ensure we have the latest key
  try {
    initializeGemini();
  } catch (e) {
    // If initialization fails (no key), rethrow specifically or handle
    throw e;
  }
  ...
  try {
    ...
    // If it's an auth error, it might be caught here
    throw e; 
  }
};

export const startPlanningSession = async (inputs: UserInputs): Promise<string> => {
  // Always initialize to get fresh key
  initializeGemini();
...
**JSON 구조 (Schema Example):**
{
  ...
  "developmentGuide": {
    ...
    "folderStructure": "**CRITICAL**: index.html MUST be at the PROJECT ROOT. Do not put it in public/.",
    "vercelDeploymentConfig": {
       "instruction": "Vercel 배포 시 화면이 안 나오는 문제(Blank Page)를 방지하기 위한 설정",
       "vercelJson": "{ \"rewrites\": [{ \"source\": \"/(.*)\", \"destination\": \"/index.html\" }] }",
       "filePlacement": "index.html은 Root에 위치하며, 내부에 <script type='module' src='/src/main.tsx'></script> 필수 포함",
       "buildSettings": "Output Directory: dist"
    },
    "imageConfiguration": {
       "storagePath": "src/assets/images",
       "manifestFile": "src/assets/images/images.txt"
    }
  },
  "imageAssets": [
    {
