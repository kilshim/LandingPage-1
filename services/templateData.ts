...
## Vercel 배포 시 404 에러 해결 방법

### 증상 1: 배포 직후 접속 시 404 Not Found (Code: NOT_FOUND)
**원인**: 
1. **\`index.html\` 위치 오류**: Vite 프로젝트에서 \`index.html\`은 \`public/\` 폴더가 아닌 **프로젝트 최상위 루트(Root)**에 있어야 합니다.
2. **Output Directory 오류**: Vercel 설정의 Output Directory가 \`dist\`(Vite 기본값)가 아닐 수 있습니다.

**해결**:
1. \`index.html\`을 프로젝트 최상위 경로로 이동.
2. Vercel 대시보드 > Project Settings > Build & Development Settings > Output Directory를 \`dist\`로 설정.

### 증상 2: 메인 페이지는 나오는데 새로고침하거나 서브 경로 진입 시 404
**원인**: SPA(Single Page Application) 라우팅 설정(\`vercel.json\`) 누락.

**해결**:
프로젝트 루트에 \`vercel.json\` 파일을 만들고 다음 내용을 입력하세요:
\`\`\`json
{
  "rewrites": [
